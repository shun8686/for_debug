#!/bin/bash

sglang_source_path=$1
test_set_file=$2
image=$3

if [ "$#" -lt 3 ];then
  echo "Param num is less than 3. Exit."
  exit 1
fi

echo "===========================SINGLE-NODE=================================="
MAX_THREADS=$(grep -v "#" server.list | wc -l)

SCRIPT_PATH=$(dirname $(readlink -f $0))
cd $SCRIPT_PATH

test_set=`cat ${test_set_file}`
echo "Run test cases: ${test_set}"
for tc_info in ${test_set}
do
    npu_num=$(echo $tc_info | cut -d'|' -f1)
    test_case=$(echo $tc_info | cut -d'|' -f2)
    while true
    do
        p_num=$(ps -ef | grep "bash run_single_base.sh " | grep -v grep | wc -l)
        if ([ ${p_num} -lt ${MAX_THREADS} ]);then
	    break
        fi
        sleep 60
    done
    echo "testcase : ${test_case}"
    nohup bash run_single_base.sh ${sglang_source_path} ${test_case} ${npu_num} ${image} > log/${test_case##*/}.log 2>&1 &
    sleep 5
done



# check result
while true
do
    p_num=$(ps -ef | grep "bash run_single_base.sh " | grep -v grep | wc -l)
    if ([ ${p_num} -eq 0 ]);then
        break
    fi
    sleep 60
done

failed_test_cases=${test_set_file}.failed
poor_test_cases=${test_set_file}.poor
> ${failed_test_cases}
> ${poor_test_cases}
for tc_info in ${test_set}
do
    test_case=$(echo $tc_info | cut -d'|' -f2)
    result=$(cat log/${test_case##*/}.log | grep "Serving Benchmark Result" | wc -l)
    ok_num=$(cat log/${test_case##*/}.log | grep "^OK$" | wc -l)
    if [ ${result} -lt 2 ];then
        echo "${tc_info}" >> ${failed_test_cases}
    elif [ "${ok_num}" -ne 1 ];then
        echo "${tc_info}" >> ${poor_test_cases} 
    fi
done


